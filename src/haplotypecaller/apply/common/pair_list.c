 ...
 *
 * @param list_block
 * @return p_hc_shared_lib_pair_list_head
 */
p_hc_shared_lib_pair_list_head hc_shared_pair_list_init(int list_block)
{
    p_hc_shared_lib_pair_list_head ret = malloc(sizeof(hc_shared_lib_pair_list_head));
    ...
    ret->nodes = 0;
    ret->pool = ring_mempool_auto_enlarge_init(sizeof(hc_shared_lib_pair_list_item), list_block);
...
int hc_shared_pair_list_insert(p_hc_shared_lib_pair_list_head head, void* first_data, void* second_data)
{
    ...
    one_node->first_data = first_data;
    one_node->second_data = second_data;
...
{
    p_hc_shared_lib_pair_list_item el;

    CDL_FOREACH(head->nodes, el)
    {
        if (el->first_data == first_data && el->second_data == second_data) {
            return 0;
        }
    }
    return 0;
...
{
    p_hc_shared_lib_pair_list_item el, tmp1, tmp2;
