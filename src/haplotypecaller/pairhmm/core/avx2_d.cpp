...
using SimdType = __m256d;  // 并行向量类型
using MaskType = uint64_t;
using UnionType = mix_D256;
...
using ProbalityArrays = ProbalityPtrDataAvx2D;

#define vec_set1_val_i(v) _mm256_set1_epi64x(v)
#define vec_set1_val_f(v) _mm256_set1_pd(v)
#define vec_add(v1, v2)   _mm256_add_pd(v1, v2)
#define vec_sub(v1, v2)   _mm256_sub_pd(v1, v2)
#define vec_mul(v1, v2)   _mm256_mul_pd(v1, v2)
#define vec_div(v1, v2)   _mm256_div_pd(v1, v2)
#define vec_set_lse(v)    _mm256_set_pd(zero, zero, zero, v)

static void precompute_masks_avx2_d(const TestCase &tc, uint32_t cols, uint32_t num_mask_vecs, MaskType (*mask_arr)[s_num_distinct_chars])
{
...
static void initialize_vectors_avx2_d(uint32_t rows, uint32_t cols, MainType *shift_out_m, MainType *shift_out_x, MainType *shift_out_y,
...
static void stripe_initialization_avx2_d(uint32_t stripe_idx, SimdType &p_gapm, SimdType &p_mm, SimdType &p_mx, SimdType &p_xx,
...
static inline void init_masks_for_row_avx2_d(const TestCase &tc, uint8_t *rs_arr, BitMaskVec &last_mask_shift_out, uint32_t begin_row_index,
...
// 每个 cc 文件单独修改
void update_masks_for_cols_avx2_d(uint32_t mask_index, BitMaskVec &bit_mask_vec, MaskType (*mask_arr)[s_num_distinct_chars],
                                  const uint8_t *rs_arr, BitMaskVec &last_mask_shift_out)
{
    MaskType *arr = mask_arr[mask_index];

    auto val0 = static_cast<int64_t>(arr[rs_arr[s_rbi_0]]);
    auto val1 = static_cast<int64_t>(arr[rs_arr[s_rbi_1]]);
    auto val2 = static_cast<int64_t>(arr[rs_arr[s_rbi_2]]);
    auto val3 = static_cast<int64_t>(arr[rs_arr[s_rbi_3]]);

    BitMaskVec src = _mm256_set_epi64x(val3, val2, val1, val0);
    BitMaskVec right_shift = _mm256_set_epi64x(s_mc_3, s_mc_2, s_mc_1, s_mc_0);
    BitMaskVec mask_vec = _mm256_srlv_epi64(src, right_shift);
    bit_mask_vec = _mm256_or_si256(mask_vec, last_mask_shift_out);

    BitMaskVec reserved_mask = _mm256_set_epi64x(s_rm_3, s_rm_2, s_rm_1, s_rm_0);
    BitMaskVec reserved_src = _mm256_and_si256(src, reserved_mask);
    BitMaskVec left_shift = _mm256_set_epi64x(s_sc_1D, s_sc_1E, s_sc_1F, s_sc_20);
    last_mask_shift_out = _mm256_sllv_epi64(reserved_src, left_shift);
}

// 每个 cc 文件单独修改
static inline void compute_dist_vec_avx2_d(BitMaskVec &bit_mask_vec, SimdType &distm_chosen, const SimdType &distm,
...
static inline void compute_mxy_avx2_d(SimdType &m_t, SimdType &m_t_y, SimdType &x_t, SimdType &y_t, const SimdType &m_t_2,
...
// 每个 cc 文件单独修改
static inline void vector_shift_avx2_d(SimdType &x, MainType shift_in, MainType &shift_out)
{
...
// 每个 cc 文件单独修改
static inline void vector_shift_last_avx2_d(SimdType &x, MainType shift_in)
{
    SimdType reversed_x = _mm256_permute4x64_pd(x, 0b10010011);
    x = _mm256_blend_pd(reversed_x, _mm256_set1_pd(shift_in), 0b0001);
}
...
double compute_fp_avx2_d(const TestCase &tc)
{
    ...
    precompute_masks_avx2_d(tc, cols, num_mask_vecs, mask_arr);
    ...
    initialize_vectors_avx2_d(rows, cols, shift_out_m, shift_out_x, shift_out_y, ctx, tc, pa, distm1d_arr);

    for (uint32_t i{0}; i < stripe_cnt - 1; ++i) {
        stripe_initialization_avx2_d(i, p_gapm, p_mm, p_mx, p_xx, p_my, p_yy, distm, _1_distm, distm1d_arr, pa, m_t_1, m_t_2, x_t_1, x_t_2,
                                     y_t_1, y_t_2, m_t_1_y, shift_out_x, shift_out_m, tc);

        init_masks_for_row_avx2_d(tc, rs_arr, last_mask_shift_out, i * s_avx_length + 1, s_avx_length);
        ...
        for (uint32_t begin_d{1}; begin_d < cols + s_avx_length; begin_d += s_main_type_size) {
            ...
            update_masks_for_cols_avx2_d((begin_d - 1) / s_main_type_size, bit_mask_vec, mask_arr, rs_arr, last_mask_shift_out);

            for (uint32_t mbi{0}; mbi < num_mask_bits_to_process; ++mbi) {
                shift_idx = begin_d + mbi + s_avx_length;

                compute_dist_vec_avx2_d(bit_mask_vec, distm, _1_distm, distm_chosen);

                compute_mxy_avx2_d(m_t, m_t_y, x_t, y_t, m_t_2, x_t_2, y_t_2, m_t_1, x_t_1, m_t_1_y, y_t_1, p_mm, p_gapm, p_mx, p_xx, p_my,
                                   p_yy, distm_chosen);

                vector_shift_avx2_d(m_t, shift_out_m[shift_idx], shift_out_m[begin_d + mbi]);
                vector_shift_avx2_d(x_t, shift_out_x[shift_idx], shift_out_x[begin_d + mbi]);
                vector_shift_avx2_d(y_t_1, shift_out_y[shift_idx], shift_out_y[begin_d + mbi]);
...

    {
        ...
        init_masks_for_row_avx2_d(tc, rs_arr, last_mask_shift_out, i * s_avx_length + 1, remaining_rows);
        ...
        for (uint32_t begin_d{1}; begin_d < cols + remaining_rows - 1; begin_d += s_main_type_size) {
            ...
            for (uint32_t mbi{0}; mbi < num_mask_bits_to_process; ++mbi) {
                ...
                vector_shift_last_avx2_d(m_t, shift_out_m[shift_idx]);

                sum_x = vec_add(sum_x, x_t);
                vector_shift_last_avx2_d(x_t, shift_out_x[shift_idx]);

                vector_shift_last_avx2_d(y_t_1, shift_out_y[shift_idx]);
