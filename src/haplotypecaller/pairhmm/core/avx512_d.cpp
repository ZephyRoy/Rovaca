...
using MainType = double;   // 精度类型
using SimdType = __m512d;  // 并行向量类型
using MaskType = uint64_t;
using UnionType = mix_D512;
...
constexpr MaskType s_mask_all_ones = s_mask_all_ones_d;
...
using ProbalityArrays = ProbalityPtrDataAvx512D;

#define vec_set1_val_i(v) _mm512_set1_epi64(v)
#define vec_set1_val_f(v) _mm512_set1_pd(v)
#define vec_add(v1, v2)   _mm512_add_pd(v1, v2)
#define vec_sub(v1, v2)   _mm512_sub_pd(v1, v2)
#define vec_mul(v1, v2)   _mm512_mul_pd(v1, v2)
#define vec_div(v1, v2)   _mm512_div_pd(v1, v2)
#define vec_set_high(v)   _mm512_set_pd(v, zero, zero, zero, zero, zero, zero, zero)

static void precompute_masks_avx512_d(const TestCase &tc, uint32_t cols, uint32_t num_mask_vecs, MaskType (*mask_arr)[s_num_distinct_chars])
{
...
static void initialize_vectors_avx512_d(uint32_t rows, uint32_t cols, MainType *shift_out_m, MainType *shift_out_x, MainType *shift_out_y,
...
static void stripe_initialization_avx512_d(uint32_t stripe_idx, SimdType &p_gapm, SimdType &p_mm, SimdType &p_mx, SimdType &p_xx,
...
static inline void init_masks_for_row_avx512_d(const TestCase &tc, uint8_t *rs_arr, BitMaskVec &last_mask_shift_out,
...
// 每个 cc 文件单独修改
static void update_masks_for_cols_avx512_d(uint32_t mask_index, BitMaskVec &bit_mask_vec, MaskType (*mask_arr)[s_num_distinct_chars],
                                           const uint8_t *rs_arr, BitMaskVec &last_mask_shift_out)
{
    ...
    BitMaskVec src = _mm512_set_epi64(val0, val1, val2, val3, val4, val5, val6, val7);
    BitMaskVec right_shift = _mm512_set_epi64(s_mc_0, s_mc_1, s_mc_2, s_mc_3, s_mc_4, s_mc_5, s_mc_6, s_mc_7);
    BitMaskVec mask_vec = _mm512_srlv_epi64(src, right_shift);
    bit_mask_vec = _mm512_or_si512(mask_vec, last_mask_shift_out);

    BitMaskVec reserved_mask = _mm512_set_epi64(s_rm_0, s_rm_1, s_rm_2, s_rm_3, s_rm_4, s_rm_5, s_rm_6, s_rm_7);
    BitMaskVec reserved_src = _mm512_and_si512(src, reserved_mask);
    BitMaskVec left_shift = _mm512_set_epi64(s_sc_40, s_sc_3F, s_sc_3E, s_sc_3D, s_sc_3C, s_sc_3B, s_sc_3A, s_sc_39);
    last_mask_shift_out = _mm512_sllv_epi64(reserved_src, left_shift);
}

// 每个 cc 文件单独修改
static inline void compute_dist_vec_avx512_d(BitMaskVec &bit_mask_vec, SimdType &distm_chosen, const SimdType &distm,
                                             const SimdType &_1_distm)
{
    ...
    bit_mask_vec = _mm512_slli_epi64(bit_mask_vec, 0x1);
}

static inline void compute_mxy_avx512_d(SimdType &m_t, SimdType &m_t_y, SimdType &x_t, SimdType &y_t, const SimdType &m_t_2,
...
// 每个 cc 文件单独修改
static inline void vector_shift_avx512_d(SimdType &x, MainType shift_in, MainType &shift_out)
{
    shift_out = _mm512_cvtsd_f64(x);

    IF_32 shiftInH;
    shiftInH.f = shift_in;
    x = _mm512_castsi512_pd(_mm512_alignr_epi64(_mm512_set1_epi64(shiftInH.i), _mm512_castpd_si512(x), 0x1));
}

// 每个 cc 文件单独修改
static inline void vector_shift_last_avx512_d(SimdType &x, MainType shift_in)
{
    IF_32 shiftInH;
    shiftInH.f = shift_in;
    x = _mm512_castsi512_pd(_mm512_alignr_epi64(_mm512_set1_epi64(shiftInH.i), _mm512_castpd_si512(x), 0x1));
...
double compute_fp_avx512_d(const TestCase &tc)
{
    ...
    precompute_masks_avx512_d(tc, cols, num_mask_vecs, mask_arr);
    ...
    initialize_vectors_avx512_d(rows, cols, shift_out_m, shift_out_x, shift_out_y, ctx, tc, pa, distm1d_arr);

    for (uint32_t i{0}; i < stripe_cnt - 1; ++i) {
        stripe_initialization_avx512_d(i, p_gapm, p_mm, p_mx, p_xx, p_my, p_yy, distm, _1_distm, distm1d_arr, pa, m_t_1, m_t_2, x_t_1,
                                       x_t_2, y_t_1, y_t_2, m_t_1_y, shift_out_x, shift_out_m, tc);

        init_masks_for_row_avx512_d(tc, rs_arr, last_mask_shift_out, i * s_avx_length + 1, s_avx_length);
        ...
        for (uint32_t begin_d{1}; begin_d < cols + s_avx_length; begin_d += s_main_type_size) {
            ...
            update_masks_for_cols_avx512_d((begin_d - 1) / s_main_type_size, bit_mask_vec, mask_arr, rs_arr, last_mask_shift_out);

            for (uint32_t mbi{0}; mbi < num_mask_bits_to_process; ++mbi) {
                shift_idx = begin_d + mbi + s_avx_length;

                compute_mxy_avx512_d(m_t, m_t_y, x_t, y_t, m_t_2, x_t_2, y_t_2, m_t_1, x_t_1, m_t_1_y, y_t_1, p_mm, p_gapm, p_mx, p_xx,
                                     p_my, p_yy, distm_chosen);

                compute_mxy_avx512_d(m_t, m_t_y, x_t, y_t, m_t_2, x_t_2, y_t_2, m_t_1, x_t_1, m_t_1_y, y_t_1, p_mm, p_gapm, p_mx, p_xx,
                                     p_my, p_yy, distm_chosen);

                vector_shift_avx512_d(m_t, shift_out_m[shift_idx], shift_out_m[begin_d + mbi]);
                vector_shift_avx512_d(x_t, shift_out_x[shift_idx], shift_out_x[begin_d + mbi]);
                vector_shift_avx512_d(y_t_1, shift_out_y[shift_idx], shift_out_y[begin_d + mbi]);
...

    {
        ...
        init_masks_for_row_avx512_d(tc, rs_arr, last_mask_shift_out, i * s_avx_length + 1, remaining_rows);
        ...
        for (uint32_t begin_d{1}; begin_d < cols + remaining_rows - 1; begin_d += s_main_type_size) {
            ...
            for (uint32_t mbi{0}; mbi < num_mask_bits_to_process; ++mbi) {
                ...
                compute_dist_vec_avx512_d(bit_mask_vec, distm_chosen, distm, _1_distm);

                compute_mxy_avx512_d(m_t, m_t_y, x_t, y_t, m_t_2, x_t_2, y_t_2, m_t_1, x_t_1, m_t_1_y, y_t_1, p_mm, p_gapm, p_mx, p_xx,
                                     p_my, p_yy, distm_chosen);

                sum_m = vec_add(sum_m, m_t);
                vector_shift_last_avx512_d(m_t, shift_out_m[shift_idx]);

                sum_x = vec_add(sum_x, x_t);
                vector_shift_last_avx512_d(x_t, shift_out_x[shift_idx]);

                vector_shift_last_avx512_d(y_t_1, shift_out_y[shift_idx]);
